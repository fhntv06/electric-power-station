...
public class ScenarioOPN : MonoBehaviour
{
    ...
    // задержка для вкл телефона ++ 
    // 1) появление ++ -> после задержки во времени идет звонок ++ -> ответ на звонок ++ -> постановка задания ++ ->
    // 2) идем на обход в первый раз, но в первый раз всё нормально -> вернуться отметиться, что всё нормально ->
    // 3) идем второй раз на обход -> находим аварию -> сообщаем о аварии   
}
